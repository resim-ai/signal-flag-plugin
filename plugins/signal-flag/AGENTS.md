...
`commands/` contains the workflow entry point and explicit reader setup command.
`scripts/setup_reader.py` verifies the bundled `reader/` wheel, dependency lock
and provenance manifest, then creates an isolated Python 3.12 inspection runtime.
`scripts/tests/test_setup_reader.py` covers integrity and bootstrap isolation.
The backend owns the reader source and release artifacts; do not patch wheel
